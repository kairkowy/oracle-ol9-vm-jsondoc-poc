...
# Doris FE/BE VM
sudo systemctl status doris-fe       # FE VM
sudo systemctl status doris-be       # BE VM
...
시작은 `PostgreSQL → MinIO → Polaris → Doris FE → Doris BE → App`, 중지는 역순입니다. Oracle은 외부 기존 DB이므로 별도 운영 절차를 따릅니다. systemd의 자동 재시작은 단일 프로세스 장애만 처리하며 의존 서비스의 데이터 일관성을 대신하지 않습니다.
...
| 계층 | POC | 운영 권고 출발점 |
|---|---|---|
| Doris | FE 1 + BE 1 | FE 3 + BE 3 이상, FQDN 등록, rack 분산 |
| Polaris | 1 node | 2+ node, LB, 외부 secret manager |
| PostgreSQL | 1 node | HA/backup/PITR |
| MinIO | 1 node/1 disk | distributed erasure coding, 별도 장애 도메인 |
| App | 1 node | 2+ node + TLS LB + 인증 |
